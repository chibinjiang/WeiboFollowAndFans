...
def add_jobs(rconn):
    users = start_nodes = ['1839190372', '1679387303', '2274552170', '1737522625', '2010873464', '1907060853']
    for uid in users:
        for title in ['FANS']:
            for page in range(1, 21):
                if page == 1:
                    url = XHR_URL.format(uid=uid, title=title, page=page)[:-7]
                else:
...
def run_all_worker():
    ...
    if not r.llen(MAD_MAN_URLS):
        add_jobs(r)
        print "Add jobs DONE, and I quit..."
        return 0
    else:
        print "Redis has %d records in cache" % r.llen(MAD_MAN_URLS)
